...
let AlgorithmHandlerComponent = class AlgorithmHandlerComponent {
    constructor(componentFactoryResolver, arrayHandler) {
    ...
    loadComponent() {
        const componentFactory = this.componentFactoryResolver.resolveComponentFactory(componentMap[this.strategy]);
...
AlgorithmHandlerComponent.ctorParameters = () => [
...
let AlgDirective = class AlgDirective {
    constructor(viewContainerRef) {
        this.viewContainerRef = viewContainerRef;
    }
};
AlgDirective.ctorParameters = () => [
...
let ArrayHandlerService = class ArrayHandlerService {
    ...
    readData(strategy, reader, tail = '') {
        reader.read().then(({ done, value }) => {
            // read data by chunks and remember tail
            const valueStr = (!value ? tail : (tail + decoder.decode(value))).split(/[\r\n]+/);
    ...
    getModifiers(strategy) {
        const ret = [
            Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["toArray"])(),
            Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])(val => Array.isArray(val) && val.reduce((acc, cur) => acc.concat([...cur]), []).map(a => parseInt(a, 10)))
        ];
        // algoritm handlers
        switch (strategy) {
            case 'simple':
...
let BigdataMedianComponent = class BigdataMedianComponent {
    constructor() { }
};
...
let LazyMedianComponent = class LazyMedianComponent {
    constructor() { }
};
...
function getMedianOfMedians(sequence$, position) {
    return new Promise((resolve, reject) => {
        ...
        const right = [];
        const medians = [];
        const medSubj = new rxjs__WEBPACK_IMPORTED_MODULE_1__["Subject"]();
        const medians$ = medSubj.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["skip"])(1), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["tap"])(a => medians.push(a)), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["last"])());
        const sourceObs = sequence$.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["bufferCount"])(5), 
        // tap(a => console.log(a)),
        Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["flatMap"])(a => {
            if (Array.isArray(a)) {
                a.sort();
                medSubj.next(a[Math.floor(a.length / 2)]);
            }
            return Object(rxjs__WEBPACK_IMPORTED_MODULE_1__["from"])(a);
        }), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["bufferWhen"])(() => medians$));
        sourceObs.subscribe(numbers => {
            ...
            if (position < k) {
                getMedianOfMedians(Object(rxjs__WEBPACK_IMPORTED_MODULE_1__["from"])(left), position).then(result => resolve(result));
            }
            else if (position > k) {
                getMedianOfMedians(Object(rxjs__WEBPACK_IMPORTED_MODULE_1__["from"])(right), position - k - 1).then(result => resolve(result));
            }
            else {
                resolve(pivot);
            }
...
let SimpleMedianComponent = class SimpleMedianComponent {
    constructor() { }
};
...
let SliceMedianComponent = class SliceMedianComponent {
    constructor() { }
};
